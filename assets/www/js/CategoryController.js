...
function CategoryController($scope, $log, $timeout, $rootScope, $location, $navigate, dateService){
    ...
    $scope.initCategoryJS = function(){
        var promise = $.Deferred($scope.serverService.getCategoryList()).promise();
        $.when(promise).then(function(data){
           $scope.imageCategories = data['imageCategories'];
        });
    }

    $scope.fetchDataForCategories = function(){
        console.log('fetchDataForCats');
        var promise = $.Deferred($scope.serverService.getDataForCategories($scope.selectedImageCategories)).promise();
        var promise2 = $.when(promise).then(function(data){
            //$scope.initDiaryJS(data);
            $scope.dateData = data;
